...
#define SJA1124_DELAY	(1000000)  //Approx 100ms Delay on FRDM-MCXC444
#define TRANSFER_BAUDRATE 500000U /* Transfer baudrate - 500k */
#define UPDATED_DUTYCYCLE 50U /* Duty Cycle */
		...
		Lin_sja1124_spi_transceiverhandle_t *pSensorHandle, ARM_DRIVER_SPI *pBus, uint8_t index, void *pSlaveSelect)
{
	int32_t status;
	uint32_t reg;
	ARM_DRIVER_GPIO *pGPIODriver = &Driver_GPIO_PORTD;

	/*! Initialize the SPI driver. */
	DRIVER_MASTER_SPI.Initialize(SPI1_SignalEvent_t);
	DRIVER_MASTER_SPI.PowerControl(ARM_POWER_FULL);
	DRIVER_MASTER_SPI.Control(ARM_SPI_MODE_MASTER | ARM_SPI_CPOL0_CPHA1, TRANSFER_BAUDRATE);
	...
	pSensorHandle->slaveParams.pTargetSlavePinID = (ARM_GPIO_Pin_t)pSlaveSelect;
	...
	pGPIODriver->Setup(pSensorHandle->slaveParams.pTargetSlavePinID, NULL);
	pGPIODriver->SetDirection(pSensorHandle->slaveParams.pTargetSlavePinID, ARM_GPIO_OUTPUT);

	if (pSensorHandle->slaveParams.ssActiveValue == SPI_SS_ACTIVE_LOW)
	{
		pGPIODriver->SetOutput(pSensorHandle->slaveParams.pTargetSlavePinID,1);
	}
	else
	{
		pGPIODriver->SetOutput(pSensorHandle->slaveParams.pTargetSlavePinID,0);
	}
...
int32_t SJA1124_Start_Pll_Clk(uint32_t pwmFreqHz)
{
	tpm_config_t tpmInfo;
	tpm_chnl_pwm_signal_param_t tpmParam;

	TPM_GetDefaultConfig(&tpmInfo);
	tpmInfo.prescale = TPM_CalculateCounterClkDiv(BOARD_TPM_BASEADDR, pwmFreqHz, TPM_SOURCE_CLOCK);
	TPM_Init(BOARD_TPM_BASEADDR, &tpmInfo);
	tpmParam.chnlNumber = (tpm_chnl_t)BOARD_TPM_CHANNEL;
	tpmParam.level      = TPM_LED_ON_LEVEL;
	tpmParam.dutyCyclePercent = UPDATED_DUTYCYCLE;
	if (kStatus_Success !=
			TPM_SetupPwm(BOARD_TPM_BASEADDR, &tpmParam, 1U, kTPM_CenterAlignedPwm, pwmFreqHz, TPM_SOURCE_CLOCK))
	{
		PRINTF("\r\nSetup PWM fail!\r\n");
		return SJA1124_FAIL;
	}
