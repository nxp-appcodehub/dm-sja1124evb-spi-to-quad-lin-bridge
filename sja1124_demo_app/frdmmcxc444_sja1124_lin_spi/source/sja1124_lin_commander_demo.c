...
#define SJA1124_INT_GPIO_INTERFACE  Driver_GPIO_PORTA
#define SJA1124_CS 4U
...
#if (RTE_SPI1_DMA_EN)
#define EXAMPLE_DMAMUX_BASEADDR 	DMAMUX0
...
extern uint8_t sja1124_spiWrite_CmdDataBuffer[64];
...
static void SJA1124_INT_ISR(uint32_t pin, uint32_t event)
{
	if(pin == SJA1124_INT_PIN && event == ARM_GPIO_TRIGGER_FALLING_EDGE)
	{
...
static void SendLinFrame(Lin_sja1124_LinChannelType LinChannel)
{
	...
	{
		SCANF("%x", &temp_value);
		DataToSend[p]=(uint8_t)(temp_value);
		PRINTF("\r\nData[%d] = 0x%x\r\n", p, DataToSend[p]);
	}
...
static void SetCommanderBreakLength()
{
	...
	{
		PRINTF("\r\nEnter your choice :- ");
		SCANF("%d",&tmp_character);
		character=(uint8_t)(tmp_character);
		PRINTF("%d\r\n",character);
...
int main(void)
{
	...
	status = SJA1124_SPI_Initialize(&spiHandle, &DRIVER_MASTER_SPI, SPI_S_DEVICE_INDEX, (void *)SJA1124_CS);
	...
	SJA1124_INT_GPIO_INTERFACE.Setup(SJA1124_INT_PIN, SJA1124_INT_ISR);
	SJA1124_INT_GPIO_INTERFACE.SetEventTrigger(SJA1124_INT_PIN, ARM_GPIO_TRIGGER_FALLING_EDGE);

	/* default configurations of SJA1124 */
	SJA1124_get_default_config(&deviceConfig, &spiHandle);

	/*generate PLL input frequency clock for SJA1124EVB */
	SJA1124_Start_Pll_Clk(DEMO_PWM_FREQUENCY);
