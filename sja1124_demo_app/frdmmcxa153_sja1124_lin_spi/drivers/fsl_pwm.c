 ...
 *
 * All submodules should not output center aligned PWMX signal when work on local sync mode. Because in above
 * scenario, VAL1 register controls both PWM period and PWMX duty cycle. PWMA and PWMB signal will be corrupted.
 * Edge aligned PWMX signal do not have such limit.
 *
 * param base        PWM peripheral base address
 * param subModule   PWM submodule to configure
                                     ...
                                     uint16_t pwmHighPulse)
{
    ...
    switch (mode)
    {
        case kPWM_SignedCenterAligned:
            ...
            {
                assert(((base->SM[subModule].CTRL2 & PWM_CTRL2_INIT_SEL_MASK) >> PWM_CTRL2_INIT_SEL_SHIFT) !=
                       kPWM_Initialize_LocalSync);
                base->SM[subModule].VAL0 = PWM_GetComplementU16(pwmHighPulse / 2U);
                base->SM[subModule].VAL1 = pwmHighPulse / 2U;
            }
            break;
        case kPWM_CenterAligned:
            ...
            else
            {
                assert(((base->SM[subModule].CTRL2 & PWM_CTRL2_INIT_SEL_MASK) >> PWM_CTRL2_INIT_SEL_SHIFT) !=
                       kPWM_Initialize_LocalSync);
                base->SM[subModule].VAL0 = (pulseCnt - pwmHighPulse) / 2U;
                base->SM[subModule].VAL1 = (pulseCnt + pwmHighPulse) / 2U;
            }
            break;
        case kPWM_SignedEdgeAligned:
            modulo = (pulseCnt >> 1U);
 ...
 * scenario, VAL1 register controls both PWM period and PWMX duty cycle. PWMA and PWMB signal will be corrupted.
 * Edge aligned PWMX signal do not have such limit.
 * Due to edge aligned PWMX is negative true signal, need to configure PWMX active level low true to get
    ...
    PWM_Type *base, pwm_submodule_t subModule, pwm_channels_t pwmSignal, pwm_mode_t currPwmMode, uint16_t dutyCycle)
{
    ...
    pulseCnt = base->SM[subModuleSync].VAL1 - base->SM[subModuleSync].INIT + 1;
 ...
 * in submodule0. In master reload mode, PWM period is depended by period of set LDOK in submodule0 because
 * this operation will reload register. If the dead time insertion logic is enabled, the pulse period is reduced
 * by the dead time eriod specified by the user. PWM signal will not be generated if its period is less than
 * dead time duration.
 *
 * param base        PWM peripheral base address
 * param subModule   PWM submodule to configure
